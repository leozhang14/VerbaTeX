...
const RecentsCard = ({ text, liked, index }: RecentsCardsProps) => {
  ...
  const handleLike = async () => {
    ...
    if (!user) {
      toast.error("User not logged in", { autoClose: 1000, hideProgressBar: true });
      return;
    }
    ...
    try {
      ...
      toast.success("Added to favourites", { autoClose: 1000, hideProgressBar: true });
    } catch (error) {
      console.error("Error saving to favourites:", error);
      toast.error("Failed to add to favourites", { autoClose: 1000, hideProgressBar: true });
    }
  };
